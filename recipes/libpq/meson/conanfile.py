...
class LibpqConan(ConanFile):
    ...
    options_description = {
        ...
        "with_libxml2": "XML support",
        "with_xslt": "XSLT support in contrib/xml2",
    ...
    def package_info(self):
        ...
        if self.options.get_safe("with_readline") == "readline":
            ...
            self.cpp_info.components["pq"].requires.append("editline::editline")
        ...
        if self.options.with_nls:
            self.cpp_info.components["pq"].requires.append("libgettext::libgettext")
        if self.options.get_safe("with_selinux"):
            self.cpp_info.components["pq"].requires.append("libselinux::libselinux")
        if self.options.with_uuid:
            self.cpp_info.components["pq"].requires.append("util-linux-libuuid::util-linux-libuuid")
        if self.options.get_safe("with_pam"):
            self.cpp_info.components["pq"].requires.append("linux-pam::pam")
        if self.options.get_safe("with_systemd"):
            self.cpp_info.components["pq"].requires.append("libsystemd::libsystemd")
