...
from conan.tools.files import copy, download, get, load, replace_in_file, rm, rmdir, save, apply_conandata_patches, export_conandata_patches
...
class LibcurlConan(ConanFile):
    ...
    def _patch_cmake(self):
        ...
        if Version(self.version) < "8.20.0":
    ...
    def _generate_with_cmake(self):
        ...
        tc.variables["CURL_ENABLE_NTLM"] = self.options.with_ntlm_wb
    ...
    def package_info(self):
        ...
        if self.settings.os in ["Linux", "FreeBSD"]:
            ...
            if Version(self.version) >= "8.15.0":
                self.cpp_info.components["curl"].system_libs.append("iphlpapi")
